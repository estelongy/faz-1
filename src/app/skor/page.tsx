              ...
              <div>
                {clinics[0] ? (
                  <>
                    ...
                    <p className="text-slate-500 text-xs">{clinics[0].city ?? 'Türkiye'}</p>
                  </>
                ) : (
                  <p className="text-slate-500 text-xs">Klinik seçimi…</p>
                )}
                <p className="text-slate-600 text-xs mt-2 blur-sm select-none">
                  Yarın · 14:00 müsait
                </p>
              </div>
            }
          >
            <div className="space-y-3">
              {clinics.map(c => (
                <Link
                  key={c.id}
                  href={`/randevu?klinik=${c.id}`}
                  className="block p-4 rounded-xl bg-slate-800 border border-slate-700 hover:border-violet-500/50 transition-colors"
                >
                  <p className="text-white font-semibold text-sm">{c.name}</p>
                  <p className="text-slate-500 text-xs mt-0.5">{c.city ?? 'Türkiye'}</p>
                </Link>
              ))}
              <Link
                href="/randevu"
